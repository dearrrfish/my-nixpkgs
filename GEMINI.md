...
1. **Create Directory:** Generate the correct `pkgs/by-name` path.
1. **Draft Derivation:**
...
1. **Test Build:** Run `nix build .#<package-name>`.

### B. Updating Packages

1. **Check Upstream:** Verify the latest stable version.
1. **Update Derivation:** Update the `version` string and the `src` hash.
   - *Tip:* Set hash to `lib.fakeSha256` first, run build, then copy the actual hash from the error message.
1. **Verify:** Ensure the package builds and runs (`nix run .#<package-name> -- --help`).
